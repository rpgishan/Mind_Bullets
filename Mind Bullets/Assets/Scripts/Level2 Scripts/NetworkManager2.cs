...
public class NetworkManager2 : MonoBehaviour {
	const string VERSION = "0.1.2v";
	string roomName = "playRoom2";
	...
	void SpawnMe(){
		...
		if (PhotonNetwork.isMasterClient) {
			...
			me.GetComponent<PlayerController2> ().enabled = true;
		} else {
			me = (GameObject)Instantiate (player, spawnPoint.position, new Quaternion (0f, 180f, 0f, 0f));
		}
		me.GetComponent<SwitchPlayer2> ().enabled = true;
		GetComponent<PileCounts2> ().enabled = true;
	...
	void StartGame(){
		if (isSignaled) {
			isSignaled = false;
			GetComponent<GameController2> ().enabled = true;
		}
	}
}